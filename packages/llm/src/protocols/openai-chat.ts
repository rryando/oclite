...
const lowerToolMessages = Effect.fn("OpenAIChat.lowerToolMessages")(function* (message: OpenAIChatRequestMessage) {
  ...
  for (const part of message.content) {
    ...
    // This fork stores structured tool results as text/media parts; upstream
    // dev refactored these into ToolContent file parts. Behavior is identical:
    // text joins into the tool message, media lowers into image_url entries.
    const text = part.result.value.filter((item) => item.type === "text").map((item) => item.text)
    messages.push({ role: "tool", tool_call_id: part.id, content: text.join("\n") })
    const media = part.result.value.filter((item) => item.type === "media")
