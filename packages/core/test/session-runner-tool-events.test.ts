...
const result = LLMEvent.toolResult({
  ...
  result: {
    ...
    value: [
      ...
      { type: "media", data: base64, mediaType: "image/png", filename: "pixel.png" },
    ],
  },
  output: {
...
test("projects a normal text stream and records its settlement", async () => {
  ...
  expect(published.at(-1)?.data).toMatchObject({ text: "Hello" })
...
test("cancellation settles every recorded pending tool", async () => {
  const { published, publisher } = capture()
  await Effect.runPromise(publisher.publish(call))
  await Effect.runPromise(publisher.failUnsettledTools("Tool execution interrupted"))

  expect(published.find((event) => event.type === "session.next.tool.failed.1")?.data).toMatchObject({
    callID: call.id,
