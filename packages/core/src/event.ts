...
> = EventDefinition<Type, DataSchema> & {
  /** @deprecated Compatibility alias for durable.version. */
  readonly version?: number
  /** @deprecated Compatibility alias for durable.aggregate. */
  readonly aggregate?: string
}
export type Data<D extends Definition> = EventData<D>
export type Payload<D extends Definition = Definition> = Omit<EventPayload<D>, "location"> & {
  readonly location?: Location.Ref
  /** @deprecated Compatibility field retained until the EventV2 bridge is removed. */
  readonly version?: number
}
...
const decodeSerializedEvent = (event: SerializedEvent): Payload => {
  const definition = Durable.get(event.type) ?? durableRegistry.get(event.type)
...
>(input: {
  ...
  // Legacy version/aggregate metadata remains live-only until opencode cuts over
  // to definitions that explicitly opt into Core durability.
  const durable = input.durable
  const definition = Object.assign(
    Event.define({ type: input.type, schema: input.schema, ...(durable ? { durable } : {}) }),
    {
  ...
  if (durable) durableRegistry.set(Event.versionedType(input.type, durable.version), definition)
  return definition
}
...
export interface Interface {
  ...
  /** @deprecated Compatibility entry point for the current EventV2 bridge. */
  ...
  readonly sync: (listener: Subscriber) => Effect.Effect<Unsubscribe>
...
export const layerWith = (options?: LayerOptions) =>
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const publishExisting = <D extends Definition>(event: Payload<D>) => {
        const definition = registry.get(event.type)
        if (!definition)
          return Effect.die(
            new InvalidDurableEventError({ type: event.type, message: `Unknown event type ${event.type}` }),
          ) as Effect.Effect<Payload<D>>
        return publishEvent(definition, event as Payload) as Effect.Effect<Payload<D>>
      }

      return Service.of({
        publish,
        publishEvent: publishExisting,
        subscribe,
        all: streamAll,
        durable,
        listen,
...
const serviceLayer = layerWith()
export const layer = serviceLayer.pipe(Layer.provide(Database.defaultLayer))
