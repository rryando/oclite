...
const layer = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    const projectMessage = (event: unknown) => run(db, event as SessionEvent.Event)
    yield* events.project(SessionV1.Event.Created, (event) =>
      Effect.gen(function* () {
        ...
        if (event.data.info.workspaceID) {
          yield* db
            .run(sql`UPDATE workspace SET time_used = ${Date.now()} WHERE id = ${event.data.info.workspaceID}`)
            .pipe(Effect.orDie)
        }
      }),
    )
