...
export const fromCatalogModel = (
  ...
  if (
    resolved.endpoint.type === "anthropic/messages" ||
    (resolved.api.type === "aisdk" &&
      ["@ai-sdk/anthropic", "@ai-sdk/google-vertex/anthropic"].includes(resolved.api.package))
  ) {
    return Effect.succeed(
      withDefaults(resolved, AnthropicMessages.route)
        .with({
          auth:
            key === undefined
              ? Auth.none
              : resolved.api.type === "aisdk" && resolved.api.package === "@ai-sdk/google-vertex/anthropic"
                ? Auth.bearer(key)
  ...
  if (resolved.api.type === "aisdk" && ["@ai-sdk/google", "@ai-sdk/google-vertex"].includes(resolved.api.package)) {
    return Effect.succeed(
      withDefaults(resolved, Gemini.route)
        .with({
          auth:
            key === undefined
              ? Auth.none
              : resolved.api.package === "@ai-sdk/google-vertex"
                ? Auth.bearer(key)
  ...
  if (resolved.api.type === "aisdk" && resolved.api.package === "@ai-sdk/amazon-bedrock") {
    const options = resolved.api.settings
    const credentials =
      ...
      typeof options.secretAccessKey === "string"
        ? {
            ...
            sessionToken: typeof options.sessionToken === "string" ? options.sessionToken : undefined,
          }
        : undefined
    return Effect.succeed(
      withDefaults(resolved, BedrockConverse.route)
        .with({ auth: key === undefined ? BedrockConverse.sigV4Auth(credentials) : Auth.bearer(key) })
        .model({ id: resolved.api.id }),
    )
  }
  if (resolved.endpoint.type === "openai/completions") {
    return Effect.succeed(
      withDefaults(resolved, OpenAIChat.route)
  ...
  if (resolved.api.type === "aisdk" && resolved.api.url) {
    return Effect.succeed(
      withDefaults(resolved, OpenAICompatibleChat.route)
        .with({ auth: key === undefined ? Auth.none : Auth.bearer(key) })
        .model({ id: resolved.api.id }),
...
export const supported = (model: ModelV2.Info) =>
  model.endpoint.type === "openai/responses" ||
  model.endpoint.type === "openai/completions" ||
  model.endpoint.type === "anthropic/messages" ||
  ...
  Effect.gen(function* () {
    ...
    return Service.of({
      resolve: Effect.fn("SessionRunnerModel.resolve")(function* (session) {
        ...
        const defaultModel = session.model ? undefined : Option.getOrUndefined(yield* catalog.model.default())
