...
const registryLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const settleWith = Effect.fn("ToolRegistry.settle")(function* (input: ExecuteInput, advertised?: object) {
      ...
      const before = yield* plugins.trigger(
        "tool.execute.before",
      ...
      const output = (yield* plugins.trigger(
        "tool.execute.after",
        {
...
function whollyDisabled(action: string, rules: PermissionV2.Ruleset) {
  const rule = rules.findLast((rule) => Wildcard.match(action, rule.permission))
...
export const node = makeLocationNode({
  service: Service,
  layer,
  deps: [ApplicationTools.node, ToolOutputStore.node, PluginV2.node],
})
